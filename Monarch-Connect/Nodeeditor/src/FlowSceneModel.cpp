...
FlowSceneModel::removeNodeWithConnections(NodeIndex const& index)
{
  ...
  auto deleteConnections = [&](PortDirection ty) -> bool {
  ...
  bool success = deleteConnections(PortDirection::In);

  if (!success)
    return false;

  success = deleteConnections(PortDirection::Out);
  if (!success)
    return false;

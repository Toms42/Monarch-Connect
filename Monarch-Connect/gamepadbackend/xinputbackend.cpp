...
void QXInputThread::dispatch(int idx, XINPUT_GAMEPAD *state)
{
    static const struct ButtonMap {
        ...
        QGamepadBackend::GamepadButton qbutton;
    } buttonMap[] = {
        { XINPUT_GAMEPAD_DPAD_UP, QGamepadBackend::ButtonUp },
        { XINPUT_GAMEPAD_DPAD_DOWN, QGamepadBackend::ButtonDown },
        { XINPUT_GAMEPAD_DPAD_LEFT, QGamepadBackend::ButtonLeft },
        { XINPUT_GAMEPAD_DPAD_RIGHT, QGamepadBackend::ButtonRight },
        { XINPUT_GAMEPAD_START, QGamepadBackend::ButtonStart },
        { XINPUT_GAMEPAD_BACK, QGamepadBackend::ButtonSelect },
        { XINPUT_GAMEPAD_LEFT_SHOULDER, QGamepadBackend::ButtonL1 },
        { XINPUT_GAMEPAD_RIGHT_SHOULDER, QGamepadBackend::ButtonR1 },
        { XINPUT_GAMEPAD_LEFT_THUMB, QGamepadBackend::ButtonL3 },
        { XINPUT_GAMEPAD_RIGHT_THUMB, QGamepadBackend::ButtonR3 },
        { XINPUT_GAMEPAD_A, QGamepadBackend::ButtonA },
        { XINPUT_GAMEPAD_B, QGamepadBackend::ButtonB },
        { XINPUT_GAMEPAD_X, QGamepadBackend::ButtonX },
        { XINPUT_GAMEPAD_Y, QGamepadBackend::ButtonY }
    };
    ...
    if (m_controllers[idx].triggers[0] != state->bLeftTrigger) {
        ...
        if (!qFuzzyIsNull(value))
            emit m_backend->gamepadButtonPressed(idx, QGamepadBackend::ButtonL2, value);
        else
            emit m_backend->gamepadButtonReleased(idx, QGamepadBackend::ButtonL2);
    }
    if (m_controllers[idx].triggers[1] != state->bRightTrigger) {
        ...
        const double value = state->bRightTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD
                ...
                : 0.0;
        if (!qFuzzyIsNull(value))
            emit m_backend->gamepadButtonPressed(idx, QGamepadBackend::ButtonR2, value);
        else
            emit m_backend->gamepadButtonReleased(idx, QGamepadBackend::ButtonR2);
    }
    ...
    if (m_controllers[idx].axis[0][0] != x) {
        ...
        emit m_backend->gamepadAxisMoved(idx, QGamepadBackend::AxisLeftX, x);
    }
    if (m_controllers[idx].axis[0][1] != y) {
        m_controllers[idx].axis[0][1] = y;
        emit m_backend->gamepadAxisMoved(idx, QGamepadBackend::AxisLeftY, y);
    }
    ...
    if (m_controllers[idx].axis[1][0] != x) {
        ...
        emit m_backend->gamepadAxisMoved(idx, QGamepadBackend::AxisRightX, x);
    }
    if (m_controllers[idx].axis[1][1] != y) {
        m_controllers[idx].axis[1][1] = y;
        emit m_backend->gamepadAxisMoved(idx, QGamepadBackend::AxisRightY, y);
    }
}

...
FramelessWindow::FramelessWindow(QWidget *parent)
    ...
    //set opacity:
    QFile defaultStyle;
    defaultStyle.setFileName(":DefaultStyle.json");
    if(defaultStyle.open(QIODevice::ReadOnly))
    {
        QByteArray bytes = defaultStyle.readAll();
        defaultStyle.close();
        QJsonDocument document(QJsonDocument::fromJson(bytes));
        QJsonObject appStyle = document.object().find("ApplicationStyle")->toObject();
        double opacity = appStyle.find("Opacity")->toDouble();
        setWindowOpacity(opacity);
    }
    else
    {
        qDebug() << "error: stylesheet not found!";
    }

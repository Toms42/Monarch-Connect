...
void
NodeState::
addPort(PortType type, PortIndex idx)
{
    if(type == PortType::In)
        _inConnections[idx].insert(0);
    else if(type == PortType::Out)
        _outConnections[idx].insert(0);
}

void
NodeState::
removePort(PortType type, PortIndex idx)
{
    if(type == PortType::In)
        _inConnections[idx].erase();
    else if(type == PortType::Out)
        _outConnections[idx].erase();
}

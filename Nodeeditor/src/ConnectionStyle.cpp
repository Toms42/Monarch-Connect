...
ConnectionStyle::loadJsonFromByteArray(QByteArray const& byteArray)
{
    ...
    QJsonValueRef colorTable = obj["ColorTable"];
    colorTableObject = colorTable.toObject();
    ...
    CONNECTION_STYLE_READ_FLOAT(obj, RandSeed);
...
{
    if(colorTableObject.contains(typeId))
    {
        QColor res;
        QJsonValue val = colorTableObject.value(typeId);
        if(val.isArray() && val.toArray().size() == 3)
        {
            ...
            res = QColor(rgb[0], rgb[1], rgb[2]);
        }
        else
        {
            res = QColor(val.toString());
        }
        return res;
    }
